...
export default function WithdrawalRequestForm() {
    ...
    const {bankInfo, agencyBank, banks, withdraw, agencyAccountInfo, withdrawInfo} = useAppDataContext();
    ...
    useEffect(() => {
        if (withdraw) {
            setValue("amountField", withdraw.amount)
            if (isCrypto === 0) {
                setValue("amountField", withdraw.amount)
                setValue("amount", withdraw.amount_in_base)
            } else if (isCrypto === 1) {
                const formattedAmount = withdraw.amount_in_quote
                    ?.toString()
                    .replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                setValue("amountField", withdraw.amount_in_quote)
                        ...
                        <div>
                            ...
                            <form autoComplete="off" onSubmit={handleSubmit(onSubmit)} id="new-withdrawal-form">
                                <div className="flex flex-col lg:flex-row gap-9">
                                    <div
                                        className="border p-4 rounded-lg dark:border-gray-600 flex flex-col gap-5 w-full lg:w-1/2 pb-5">
                                        {/*Amount*/}
                                        <Input
                                            placeholder=""
                                            label="출금가능액"
                                        ...
                                        {watch("isCrypto") === 0 && (
                                            <>
                                                ...
                                                {agencyBank && <Select
                                                    ...
                                                    onChange={(e) => {
                                                        const selected = agencyBank.find((b) => b.id === +e.target.value);
                                                        if (selected) {
